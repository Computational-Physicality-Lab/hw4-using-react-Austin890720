...
const Cart = () => {
    ...
    const subtotal = (cartItem.reduce((acc, item) => acc + item.Quantity * parseFloat(item.Price.replace("$", "")), 0)).toFixed(2);
    const total = (parseFloat(subtotal) + 6.95).toFixed(2);
    return (
        <div className="Cart" >
            <div className="cart_header">
                {cartItem.length === 0 ? "Your Cart is Empty" : "My Cart (" + number + ")"}
            </div>
                            ...
                            <div className='element_right'>
                                <div style={{ display: 'flex', alignItems: 'center' }}>
                                    ...
                                    <div className='quantity'>
                                        <select value={quantities[i]} onChange={e => handleChange(i, e.target.value)} className='quantity_size'>
                                ...
                                <button className='remove_button' onClick={() => handleRemove(i)}>
                                    Remove
                                </button>
                ...
                <div style={{ height: "600px" }}>
                    <div className='order_summary_box'>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '20px' }} >
                            ...
                            <div style={{ float: 'right', textAlign: 'right' }} className='price_right'>
                                <p>${subtotal}</p>
                                <p>$6.95</p>
                                <hr style={{ borderTop: '2px solid red' }} />
                                <p style={{ fontSize: "1cm" }}>${total}</p>
